...
Sentry.init({
  dsn: "https://72bb2d4d52d49700e5de702919f16298@o4511196684288000.ingest.de.sentry.io/4511197549428816",
});
const {
  app,
  BaseWindow,
...
const TARGET_URL = IS_DEV
  ? "https://897dgo89roy8rgtery7t.lol"
  : "https://automint.online";
const UPDATE_URL = "https://updates.automint.online";
  ...
  ses.webRequest.onBeforeSendHeaders((details, callback) => {
    try {
      ...
      const sameOrigin =
        ...
        host === "automint.online" ||
        host.endsWith(".automint.online");
      if (!sameOrigin) {
...
function attachCleanUserAgent(view) {
  ...
  const ua =
    wc
      ...
      .replace(/\s*AutomintDesktop\/[\w.-]+/, "") +
    ` AutomintDesktop/${app.getVersion()}`;
  wc.setUserAgent(ua);
}
...
async function verifyIntegrity() {
  ...
  try {
    ...
    const versionEntry = data[app.getVersion()];
    const expected = versionEntry && versionEntry[process.platform];
    if (!expected) return;
      ...
      dialog
        .showMessageBox({
          ...
          detail:
            ...
            "For your safety, please download the official version from automint.online.",
        ...
        .then(({ response }) => {
          if (response === 0) {
            shell.openExternal("https://automint.online/download");
            isQuitting = true;
            app.quit();
          }
        });
